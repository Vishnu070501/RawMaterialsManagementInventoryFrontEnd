...
const ScrapInventoryTable = ({ data, pageNumber, onItemClick }) => {
  ...
  const columns = [
    ...
    { key: "item_name", label: "SCRAP NAME", type: "string", sticky: true, width: 200 },
    ...
    { key: "total_quantity", label: "TOTAL QUANTITY", type: "number", width: 180 },
    { key: "total_scrap_quantity", label: "SCRAP QUANTITY", type: "number", width: 180 },
    { key: "total_scrap_weight", label: "SCRAP WEIGHT", type: "string", width: 180 },
    // { key: "total_weight", label: "TOTAL WEIGHT", type: "string", width: 180 },
  ...
  const renderCustomCell = (item, column, index, extractNumericValue) => {
    ...
    if (column.key === 'sr_no') {
      const serialNumber = (pageNumber - 1) * 10 + index + 1;
      return serialNumber;
    }
    // Handle numeric columns with fixed decimal places
    else if (['thickness', 'slit_size', 'blank_size', 'total_quantity', 'total_scrap_quantity'].includes(column.key)) {
  ...
  return (
    <>
      <DataTable
        ...
        emptyMessage="No scrap inventory items found"
      />
...
export default ScrapInventoryTable;
